    ...
    <aside
      className={
        isChecklistMinimized
          ? "fixed bottom-4 right-4 z-40 w-auto max-w-[12rem] shrink-0 rounded-xl border border-border bg-panel/95 p-2.5 shadow-md backdrop-blur-sm dark:bg-panel"
          : "h-fit w-full max-w-none shrink-0 self-stretch rounded-xl border border-border bg-panel/95 p-2.5 shadow-sm backdrop-blur-sm dark:bg-panel sm:max-w-md sm:self-center lg:sticky lg:top-3 lg:max-w-[15.5rem] lg:self-start"
      }
    >
      <div className="flex items-start justify-between gap-1.5">
        <div className="min-w-0">
          ...
          <h2 className="mt-0.5 text-xs font-semibold leading-tight tracking-tight text-foreground">
            Quick onboarding
          </h2>
        ...
        <div className="flex shrink-0 items-center gap-1">
          ...
          <button
            type="button"
            onClick={() => setIsChecklistMinimized((value) => !value)}
            className="inline-flex h-6 w-6 items-center justify-center rounded-md border border-border text-xs font-semibold text-muted transition-colors hover:bg-panel-muted hover:text-foreground"
            aria-label={isChecklistMinimized ? "Expand guide" : "Minimize guide"}
            title={isChecklistMinimized ? "Expand guide" : "Minimize guide"}
          >
            {isChecklistMinimized ? "+" : "-"}
          </button>
        </div>
      ...
      {!isChecklistMinimized ? (
        <ul className="mt-2 space-y-1.5">
          {checklistItems.map((item) => (
            <li key={item.id} className="rounded-lg border border-border bg-panel-muted/40 px-2 py-2">
              {(() => {
                ...
                const actionButtonClass =
                  "inline-flex h-7 items-center rounded-md border border-border px-2 text-[10px] font-semibold leading-none transition-colors";
                  ...
                  <>
                    <div className="flex min-w-0 items-start gap-1.5">
                      {item.completed ? (
                        ...
                        <Circle className="mt-0.5 h-3 w-3 shrink-0 text-muted" />
                      )}
                    ...
                    {expandedGuideId === item.id && unlocked ? (
                      <div className="mt-1.5 max-h-40 overflow-y-auto rounded-md border border-border bg-panel px-1.5 py-1.5">
                        <p className="text-[9px] leading-snug text-muted">
                          <span className="font-semibold text-foreground">What:</span> {item.teach.what}
                        </p>
                        <p className="mt-1 text-[9px] leading-snug text-muted">
                          <span className="font-semibold text-foreground">Why:</span> {item.teach.why}
                        </p>
                        <p className="mt-1 text-[9px] leading-snug text-muted">
                          <span className="font-semibold text-foreground">How:</span> {item.teach.how}
                        </p>
                      </div>
                    ) : null}
                  </>
        ...
        <p className="mt-2 text-[10px] text-muted">Guide minimized. Expand anytime.</p>
      )}
    </aside>
  );
      ...
      >
        {spaces.length === 0 ? (
          <section className="px-4 py-4 sm:px-6 sm:py-6">
            <div
              className={`mx-auto flex w-full max-w-6xl flex-col gap-6 sm:gap-8 ${
                hideChecklist ? "" : "lg:flex-row lg:items-start lg:justify-between"
              }`}
            >
              <div className="grid min-w-0 flex-1 place-items-center px-2 py-6 sm:py-10 lg:min-h-[min(28rem,50vh)]">
          ...
          <section className="px-4 py-4 sm:px-6 sm:py-6">
            ...
            >
              <div className="grid min-w-0 flex-1 gap-4 sm:gap-5 lg:min-w-0 xl:grid-cols-2">
                {spaces.map((space) => (
                  <Link
                    key={space.id}
