...
export function UpgradeRequiredModal({ prompt, onClose }: UpgradeRequiredModalProps) {
  const isMemberLimitPrompt =
    (prompt?.title ?? "").toLowerCase().includes("member") ||
    (prompt?.reason ?? "").toLowerCase().includes("team member");

  return (
    <Modal
      open={Boolean(prompt)}
      onClose={onClose}
      title={prompt?.title ?? "Need a little more room?"}
      description="Your current plan does not include enough capacity for this action. You can move up a tier whenever you are ready."
      ...
      <div className="space-y-4 px-5 py-5">
        <div className="rounded-[1.4rem] border border-border bg-panel-muted px-4 py-4">
          ...
          {isMemberLimitPrompt ? (
            <p className="mt-2 text-xs leading-5 text-muted">
              Note: roster limits include the account owner or admins on the space team.
            </p>
          ) : null}
        </div>
        ...
        <div className="rounded-[1.2rem] border border-accent/25 bg-accent-soft/50 px-4 py-3 text-sm text-foreground">
          <p className="inline-flex items-center gap-2 font-medium">
            ...
            Upgrading unlocks higher limits on your next subscription invoice.
          </p>
        </div>
