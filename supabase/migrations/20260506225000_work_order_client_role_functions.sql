-- Work order assignment role: client — functions, policies, and permission seed (after enum exists).
...
as $$
  select
    public.has_work_order_permission(target_work_order_id, 'send_messages')
    and (
      public.has_work_order_permission(target_work_order_id, 'upload_files')
      or public.has_work_order_permission(target_work_order_id, 'download_files')
    );
$$;
...
using (
  ...
  and (
    ...
    or public.has_work_order_permission(work_order_id, 'download_files')
  )
);
...
as $$
  ...
  allowed(role, permission_key) as (
    values
      ...
      ('client'::public.app_role, 'send_messages'),
      ('client'::public.app_role, 'edit_own_messages'),
      ('client'::public.app_role, 'delete_own_messages')
  )
